  ...
  function inject() {
    ...
    s.textContent =
      ':root{' +
        ...
        '--gg-font:' + B.font + ';' +
      '}' +
      /* بديل الصورة المكسورة — مرسوم بلونك، لا أيقونة قبيحة */
      '.gmt-img-fallback{background:linear-gradient(135deg,' + B.red + '22,' + B.red + '0a)!important;' +
        'border:1px dashed ' + B.red + '55!important;border-radius:' + B.radius + ';' +
        'display:flex!important;align-items:center;justify-content:center;' +
        'color:' + B.red + ';font-family:' + B.font + ';font-weight:900;font-size:11px;text-align:center;' +
        'min-height:44px;padding:6px;object-fit:contain;}';
  ...
  function guardImages() {
    function handle(img) {
      ...
      img.addEventListener('error', function () {
        ...
        // استبدل بعنصر بديل بلون الهوية
        var d = document.createElement('div');
        d.className = 'gmt-img-fallback ' + (img.className || '');
        d.style.cssText = 'width:' + (img.width || img.offsetWidth || 60) + 'px;height:' + (img.height || img.offsetHeight || 44) + 'px;';
        d.title = 'صورة مفقودة: ' + name;
        d.textContent = (img.alt || name.replace(/\.\w+$/, '')).slice(0, 18);
        if (img.parentNode) img.parentNode.replaceChild(d, img);
        // أبلِغ البوتات — لا تمرّ الصور المفقودة بصمت
